...
public class RemoveDuplicatesFromSortedArrayTwo {
    ...
    public static int removeDuplicates(int[] nums) {
        ...
        final var valueThatSimulatesAnEmptyIndexElement = 999;
        ...
        while (i + 1 < nums.length) {
            if (nums[i] == nums[i + 1]) {
                ...
                if (nums[i] == nums[i + 1]) {
                    var repeatedNumberIndexToRemove = i + 1;

                    do {
                        nums[repeatedNumberIndexToRemove] = valueThatSimulatesAnEmptyIndexElement;
                        ++repeatedNumberIndexToRemove;
                    } while (repeatedNumberIndexToRemove < nums.length && nums[i] == nums[repeatedNumberIndexToRemove]);

                    i = repeatedNumberIndexToRemove;
                    continue;
                }
